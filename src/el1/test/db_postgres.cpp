...
namespace
{
	...
	TEST(db_postgres_codec, ByteArrayAndStrings)
	{
		...
		const TString text(U"Grüße from PostgreSQL");
	...
	TEST(db_postgres_codec, TimestampJsonAndNoOp)
	{
		...
		const TJsonValue json = TJsonValue::Parse(U"{\"answer\":42,\"ok\":true}");
	...
	TEST(db_postgres, simple_select)
	{
		...
		{
			usys_t n_rows = 0;
			for(auto rs = connection.Execute(U"select 1,2,3.0::float8,'hello world',NULL,true,false,timestamp '2000-01-01',timestamp '1970-01-01', '{\"num\":0}'::jsonb"); !rs->End(); rs->MoveNext())
			{
				...
				EXPECT_TRUE(s != nullptr && *s == U"hello world");
				...
				EXPECT_TRUE(js != nullptr && js->Map().Contains(U"num"));
		...
		{
			usys_t n_rows = 0;
			for(auto rs = connection.Execute(U"select oid from pg_type"); !rs->End(); rs->MoveNext())
			{
				auto [i] = rs->Row<s32_t>();
	...
	TEST(db_postgres, datatype_codecs)
	{
		...
		{
			TPostgresConnection connection(properties);
			connection.Execute(U"DROP DOMAIN IF EXISTS public.el1_test_int4_domain CASCADE")->DiscardAllRows();
			connection.Execute(U"CREATE DOMAIN public.el1_test_int4_domain AS int4")->DiscardAllRows();
		}

		{
			TPostgresConnection connection(properties);

			auto rs = connection.Execute(U"SELECT 1234::public.el1_test_int4_domain, int4out(5678)");
			...
			EXPECT_TRUE(*cstring_value == U"5678");

			rs = connection.Execute(U"SELECT pg_sleep(0)");
			auto [void_value] = rs->Row<void>();
			EXPECT_NE(void_value, nullptr);

			TList<byte_t> bytea_value = {0, 1, 2, 3, 255};
			rs = connection.Execute(U"SELECT $1::bytea", bytea_value);
			...
			rs = connection.Execute(U"SELECT $1::bytea", empty_bytea);
			auto [empty_bytea_result] = rs->Row<TList<byte_t>>();
			ASSERT_NE(empty_bytea_result, nullptr);
			EXPECT_EQ(empty_bytea_result->Count(), 0U);

			connection.Execute(U"DROP DOMAIN public.el1_test_int4_domain")->DiscardAllRows();
		}
	}
	...
	TEST(db_postgres, ltree)
	{
...

		{
			...
			try
			{
				connection.Execute(U"CREATE EXTENSION IF NOT EXISTS ltree")->DiscardAllRows();
			}
...

		{
			...
			auto rs = connection.Execute(U"SELECT $1::ltree, subpath($1::ltree, 0, 2), ''::ltree", value);
			...
			EXPECT_TRUE(full_path->ToString() == U"Top.Science.Astronomy");
			EXPECT_TRUE(parent_path->ToString() == U"Top.Science");
	...
	TEST(db_postgres, notify_channel)
	{
		...
		EXPECT_THROW(listener_connection.SubscribeNotifyChannel(U""), TInvalidArgumentException);

		auto rs = listener_connection.Execute(U"SELECT pg_backend_pid()");
		...
		const TString first_payload = U"first payload";
		const TString second_payload = U"second payload";
		...
		rs = listener_connection.Execute(U"SELECT EXISTS (SELECT 1 FROM pg_listening_channels() AS channel(name) WHERE name = $1)", channel_name);
		auto [is_listening] = rs->Row<bool>();
		ASSERT_NE(is_listening, nullptr);
		EXPECT_TRUE(*is_listening);

		sender_connection.Execute(U"SELECT pg_notify($1, $2)", channel_name, first_payload)->DiscardAllRows();
		...
		listener1.reset();
		sender_connection.Execute(U"SELECT pg_notify($1, $2)", channel_name, second_payload)->DiscardAllRows();
		ASSERT_TRUE(listener2->OnNotify()->WaitFor(5));
		...
		sender_connection.Execute(U"SELECT pg_notify($1, $2)", channel_name, empty_payload)->DiscardAllRows();
	...
	TEST(db_postgres, notify_channel_disconnect)
	{
		...
		{
			TPostgresConnection connection(properties);
			listener = connection.SubscribeNotifyChannel(U"el1_notify_disconnect_test");
			EXPECT_TRUE(listener->Name() == U"el1_notify_disconnect_test");
	...
	TEST(db_postgres, wrong_column_count)
	{
...

		{
			usys_t n_rows = 0;
			bool _throw = false;
			for(auto rs = connection.Execute(U"select 1,2,3"); !rs->End(); rs->MoveNext())
			{
				try
				{
...

		{
			usys_t n_rows = 0;
			bool _throw = false;
			for(auto rs = connection.Execute(U"select 1"); !rs->End(); rs->MoveNext())
			{
				try
				{
					auto [i,j] = rs->Row<s32_t, s32_t>();
	...
	TEST(db_postgres, bad_query_recovery)
	{
		...
		auto rs = connection.Execute(U"some nonsense text");
		EXPECT_THROW(rs->DiscardAllRows(), TPostgresException);

		rs = connection.Execute(U"select 1234");
	...
	TEST(db_postgres, pipeline)
	{
		...
		auto rs1 = connection.Execute(U"select 10");
		auto rs2 = connection.Execute(U"select 20");
		auto rs3 = connection.Execute(U"select 30");
 	...
 	TEST(db_postgres, TPostgresConnection_Prepare)
	{
		...
		auto ps = connection.Prepare(U"SELECT 1234");
		auto rs = ps->Execute();
		EXPECT_EQ(rs->CountColumns(), 1U);
		auto [i] = rs->Row<s32_t>();
		EXPECT_TRUE(i != nullptr && *i == 1234);
