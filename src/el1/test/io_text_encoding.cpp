...
namespace
{
	...
	TEST(io_text_encoding_utf8, TUTF8Decoder)
	{
		...
		// single byte ASCII (y)
		{
			...
			TList<char32_t> utf32_data = utf8_data.Pipe().Transform(TUTF8Decoder()).Collect();
			EXPECT_EQ(utf32_data.Count(), 1U);
			EXPECT_EQ(utf32_data[0], 0x79U);
		}
		...
		// two-byte sequence (ä)
		{
			...
			TList<char32_t> utf32_data = utf8_data.Pipe().Transform(TUTF8Decoder()).Collect();
			EXPECT_EQ(utf32_data.Count(), 1U);
			EXPECT_EQ(utf32_data[0], 0x00E4U);
		}
		...
		// three-byte sequence (€)
		{
			...
			TList<char32_t> utf32_data = utf8_data.Pipe().Transform(TUTF8Decoder()).Collect();
			EXPECT_EQ(utf32_data.Count(), 1U);
			EXPECT_EQ(utf32_data[0], 0x20ACU);
		}
		...
		// four-byte sequence (𝄞)
		{
			...
			TList<char32_t> utf32_data = utf8_data.Pipe().Transform(TUTF8Decoder()).Collect();
			EXPECT_EQ(utf32_data.Count(), 1U);
			EXPECT_EQ(utf32_data[0], 0x1D11EU);
		}
		...
		// mixed sequence (yä€𝄞y)
		{
			...
			EXPECT_EQ(utf32_data[0], 0x79U);
			EXPECT_EQ(utf32_data[1], 0x00E4U);
			EXPECT_EQ(utf32_data[2], 0x20ACU);
			EXPECT_EQ(utf32_data[3], 0x1D11EU);
			EXPECT_EQ(utf32_data[4], 0x79U);
		}
	...
	TEST(io_text_encoding_utf8, TUTF8Encoder)
	{
		// single byte ASCII (y)
		{
			const TList< char32_t> utf32_data = { 0x79U };
		...
		// two-byte sequence (ä)
		{
			const TList< char32_t> utf32_data = { 0x00E4U };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
		...
		// three-byte sequence (€)
		{
			const TList< char32_t> utf32_data = { 0x20ACU };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
		...
		// four-byte sequence (𝄞)
		{
			const TList< char32_t> utf32_data = { 0x1D11EU };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
		...
		// mixed sequence (yä€𝄞y)
		{
			const TList< char32_t> utf32_data = { 0x79U, 0x00E4U, 0x20ACU, 0x1D11EU, 0x79U };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
		...
		{
			const TList< char32_t> utf32_data = { 0U };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
			EXPECT_EQ(utf8_data.Count(), 1U);
		...
		{
			const TList< char32_t> utf32_data = { 0x3FFFFFU };
			TList<byte_t> utf8_data = utf32_data.Pipe().Transform(TUTF8Encoder()).Collect();
			EXPECT_EQ(utf8_data.Count(), 4U);
		...
		{
			const TList< char32_t> utf32_data = { 0x400000U };
			EXPECT_THROW(utf32_data.Pipe().Transform(TUTF8Encoder()).Collect(), TInvalidUtf8SequenceException);
		}
		{
			const TList< char32_t> utf32_data = { 0x500000U };
			EXPECT_THROW(utf32_data.Pipe().Transform(TUTF8Encoder()).Collect(), TInvalidUtf8SequenceException);
		}
		{
			const TList< char32_t> utf32_data = { 0xFFFFFFFFU };
			EXPECT_THROW(utf32_data.Pipe().Transform(TUTF8Encoder()).Collect(), TInvalidUtf8SequenceException);
		}
	}
}
