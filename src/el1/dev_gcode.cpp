...
{
	static ITextWriter& operator<<(ITextWriter& tw, const v2d_t v)
	{
		tw<<"X"<<v[0]<<" Y"<<v[1];
		return tw;
	}
	...
	void GenerateForLaserEngraver(const TJob& job, ITextWriter& tw, const float fr_min, const float fr_max, const float fr_travel, const float fr_min_grbl, const float fr_max_grbl, const float laser_scale, const float spmm)
	{
		...
		tw<<"G21\nG90\nT0\nM3 S0\n";	// mm, abs pos, tool 0, start spindel
		...
		for(auto& cmdptr : job.commands)
		{
			...
			{
				EL_THROW(TInvalidArgumentException, "job", "job contains delay command(s)");
			}
			else if(const TPauseCommand* pause_delay = dynamic_cast<const TPauseCommand*>(cmd))
			{
				tw<<"M0\n";
			}
			else
				EL_THROW(TInvalidArgumentException, "job", "job contains unknown command(s)");
		}
	...
	void GenerateForLaserEngraver(const TJob& job, ITextWriter& tw, const float feedrate, const float laser_scale)
	{
		string::TNumberFormatter nf = string::TNumberFormatter::PLAIN_DECIMAL_US_EN;
		nf.config.n_decimal_places = 6;
		nf.config.n_min_integer_places = 1;
		...
		bool laser_on = false;
		float spindel_speed = 0.0f;
		v2d_t pos(0,0);
		tw<<"G21\nG90\nT0\nM3 S0\nF"<<feedrate<<"\n";	// mm, abs pos, tool 0, start spindel, set feedrate
		...
		for(auto& cmdptr : job.commands)
		{
			...
			{
				pos = (cmd_goto->ctype == ECoordinateType::ABSOLUTE) ? cmd_goto->endpoint : (pos + cmd_goto->endpoint);
				tw<<"G1 "<<pos<<" S"<<(laser_on ? spindel_speed : 0.0f)<<"\n";
			}
			else if(const TArcMoveCommand* cmd_arc = dynamic_cast<const TArcMoveCommand*>(cmd))
			{
				pos = (cmd_arc->ctype == ECoordinateType::ABSOLUTE) ? cmd_arc->arc.endpoint : (pos + cmd_arc->arc.endpoint);
				const char* const cmd = (cmd_arc->arc.dir == TArc::EDirection::CLOCKWISE) ? "G2 " : "G3 ";
				tw<<cmd<<pos<<" I"<<cmd_arc->arc.center[0]<<" J"<<cmd_arc->arc.center[0]<<" S"<<(laser_on ? spindel_speed : 0.0f)<<"\n";
			}
			else if(const TPenCommand* cmd_pen = dynamic_cast<const TPenCommand*>(cmd))
			{
				laser_on = cmd_pen->dir == TPenCommand::EDirection::DOWN;
			}
			else if(const TColorChangeCommand* cmd_cc = dynamic_cast<const TColorChangeCommand*>(cmd))
			{
				const auto& new_color = cmd_cc->new_color;
				spindel_speed = (new_color[0] + new_color[1] + new_color[2]) / 3.0f * laser_scale;
			}
			else if(const TPauseCommand* pause_delay = dynamic_cast<const TPauseCommand*>(cmd))
			{
				tw<<"M0\n";
